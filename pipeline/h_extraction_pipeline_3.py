...
desnity_and_hydrogen_data = json.load(open('material_properties.json'))
...
def predict(img_path, is_url=False):
    ...
    for img_name in tqdm(os.listdir(current_predictions_path),desc="Classifying Objects"):
        ...
        if img_name.endswith('.png') and img_name != 'label_visualization.png':
            ...
            results[img_id]['density'] = desnity_and_hydrogen_data[class_name]['density']
            results[img_id]['hydrogen_ratio'] = desnity_and_hydrogen_data[class_name]['hydrogen_ratio']
            results[img_id]['fill_ratio'] = desnity_and_hydrogen_data[class_name].get('fill_ratio', 1)
